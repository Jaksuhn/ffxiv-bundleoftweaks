...
using static InteropSourceGenerators.DiagnosticDescriptors;
...
internal sealed class AddressHookGenerator : IIncrementalGenerator
{
    ...
    internal sealed record AddressHookInfoGeneric(MethodInfo MethodInfo, string StructName, string AddressName)
    {
        ...
        {
            builder.AppendLine($"{MethodInfo.Name}Hook = Svc.Hook.HookFromAddress<{MethodInfo.Name}Delegate>((nint){StructName}.Addresses.{AddressName}.Value, {MethodInfo.Name});");
        }
    }
